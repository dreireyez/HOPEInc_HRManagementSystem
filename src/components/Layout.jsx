...
import { useRights } from '../context/UserRightsContext'; // Use the hook
...
export default function Layout() {
  ...
  const navItems = [
    ...
    // GATED ITEMS: Now using specific Right Codes
  ...
  // Logic: Filter based on the 'can' helper
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0F] text-[#e2e2e2] relative font-body">
      {/* ... Background Orbs (Keep your existing code) ... */}
      ...
      {/* Top Navbar (Keep your existing code) */}
      <nav className="fixed top-0 w-full z-50 bg-[#16161E]/90 backdrop-blur-xl border-b border-white/5 flex justify-between items-center px-12 h-20">
        ...
        <button className="bg-white/5 border border-white/10 text-white px-6 py-2 rounded-full font-bold text-sm hover:bg-white/10 transition-all">Logout</button>
      </nav>
